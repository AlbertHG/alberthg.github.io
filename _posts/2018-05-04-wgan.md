...
在这里我们先来了解一下什么是 F 散度。设定 $P$ 和 $Q$ 是两个不同的分布， $p(x)$ 和 $q(x)$ 代表着分别从 $P$ 和 $Q$ 采样出 $x$ 的几率，则我们将F-散度定义为：
...
也就是说，只要两个分布稍有不同，就能通过 $D_f$ 得到的正值反映出来。这个时候我们发现之前常用的 KL 散度其实就是 F 散度的一种。

当你设置 $f(x) = x\ log\ x$ 即将 F 散度转换为了 KL 散度了。

$$D_f(P||Q)=\int_x q(x)\frac{p(x)}{q(x)}\log(\frac{p(x)}{q(x)})dx=\int_xp(x)\log(\frac{p(x)}{q(x)})dx$$

当你设置 $f(x) = -\ log\ x$ 即将 F 散度转换为了 KL 散度了。

$$D_f(P||Q)=\int_xq(x)(-\log(\frac{p(x)}{q(x)}))dx=\int_xq(x)\log(\frac{q(x)}{p(x)})dx$$

当你设置 $f(x) = (x-1)^2$ 即将 F 散度转换为了 KL 散度了。
...
上述公式的意思就是给定 $t$ 找出一个在 $f(x)$ 里边有定义的 $x$ 使得 $xt-f(x)$ 最大，当然 $t$ 可以无限取值，那么假定我们将 $t = t_1$ 和 $t = t_2$ 则有：
...
利用这个关系，我们能够讲 F 散度的定义变形为一个类似于 GAN 的式子。
...
- 第一行就是 F 散度的定义式；
- 第二行就是将函数 $f$ 里边的内容用 $f(x) = \underset{t \in dom(f^{* })}{max}\left \{ xt-f^{* }(t) \right \}$ 替换掉；
...
我们知道，GAN 的目的是训练生成器 G，使其产生的数据分布 $P_G$ 与真实数据的分布 $P_{data}$ 尽可能小。换言之，如果我们用 ƒ-Divergence 来表达 $P_G$ 与 $P_{data}$ 的差异，则希望最小化 $D_f(P_{data}||P_G)$ 。
...
上述从数学推导上给出了 $V(G,D)$ 的定义方式。但实际上要注意，此处的 $V(G,D)$ 不一定就是原生GAN的形式。 ƒ-Divergence GAN 是对 GAN 模型的统一，对任意满足条件的 $f$ 都可以构造一个对应的 GAN。事实上，在将其应用到GAN时，有非常多的不同的生成器函数可供使用：
...
对于传统的 GAN 来说，选定特定的度量函数之后，会导致目标函数变成生成器分布和真实分布的 JS 散度度量。但是使用JS 散度有很多问题，比如说一个最严重的问题就是当两个分布之间完全没有重叠时，分布间距离的大小并不会直接反映在散度上。这对基于迭代的优化算法是个致命问题。
...
EM 距离也可以类似 FDivergence，用一个式子表示出来：
...
。此时如果我们想要满足上面的优化目标的话，就可以让 $D(x_1)=k+d$，让$D(x_2)=k$。其中 $k$ 具体是什么无所谓，关键是我们通过d将判别器在不同分布上的结果限制在了一个较小的范围中。
...
由于我们的函数是一个“缓慢变化”的函数，想要让函数缓慢变化，只需要让权值变小一些即可:在每次参数更新之后，让每个大于 $c$ 的参数 $w$ 等于 $c$ 、让每个小于 $−c$ 的参数 $w$ 等于 $−c$，即将所有权值参数 $w$ 截断在 $[−c,c]$ 之间。然而这么做实际上保证的并不是 $1-Lipschitz$，而是 $K-Lipschitz$，甚至这个 $K$ 是多少都是玄学，只能通过调参来测试了。
